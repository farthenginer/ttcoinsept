...
#pragma warning restore 649
        ...
        /// </summary>
        private async void SetSpinCost()
        {
            var spinRemain = await GoogleAuthentication.Instance.GetRemainingSpins();
            remainText.text = $"Rmndr: {spinRemain}";
        ...
        public async void OnSpinButtonPressed()
        {
            ...
            DateTime nextTime = await GoogleAuthentication.Instance.getSpinData();
            int rmndr = await GoogleAuthentication.Instance.GetRemainingSpins();
            var numCoins = UserData.Instance.TTCoin;
            bool canSpin = await GoogleAuthentication.Instance.CanUserSpin();
            if (numCoins >= spinCost && canSpin && rmndr > 0)
            {
                if (!isFreeSpin)
                {
                    await GoogleAuthentication.Instance.Spin();
            ...
            else
            {
                parentScene.OpenPopup<AlertPopup>("Popups/AlertPopup", popup =>
                {
                    ...
                    if (!canSpin)
                    {
                        if (numCoins >= spinCost)
                        {
                            popup.SetText($"You have no other rights this week. {nextTime.Day}/{nextTime.Month}");

                        }
                        else
                        {
                            popup.SetText($"You have no other rights this week");
                    ...
                    else if (canSpin)
                    {
                        if (numCoins < spinCost)
                        {
                            popup.SetText("Insufficient TTCoin");
