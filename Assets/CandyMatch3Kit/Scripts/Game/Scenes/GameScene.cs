...
#pragma warning restore 649
        ...
        public async void CheckEndGame()
        {
            ...
            {
                EndGame();
                var nextLevel = UserData.Instance.UserLevel;
                ...
                {
                    await GoogleAuthentication.Instance.SaveOrUpdateTotalScore(gameBoard.gameState.score);
                    await GoogleAuthentication.Instance.UpdateHighScoreInFirestoreAsync(gameBoard.gameState.score);
                    RewardUser();

                    // Level sayısını PlayerPrefs ile sakla

                    GoogleAuthentication.Instance.NextLevel(true, level.id + 1);
        ...
        /// Opens the win popup.
        /// </summary>
        public async void OpenWinPopup()
        {
            ...
            // Her iki bölümde bir reklam göster
            if (levelCounter % 2 == 0)
            {
                bool status = await GoogleAuthentication.Instance.GetRemoveAdsStatus();
                if (!status)
                {
                    var applovin = FindObjectOfType<ApplovinAds>().gameObject;
                    applovin.GetComponent<ApplovinAds>().ShowInterstitial();
                }
            }
            await GoogleAuthentication.Instance.FetchLevelFromFirestore();

        }
        async void RewardUser()
        {
            PuzzleMatchManager.instance.livesSystem.RemoveLife();
            var level = UserData.Instance.UserLevel;

            if (level > 0 && level < 51)
            {
                await GoogleAuthentication.Instance.SaveTTCoin(60);
            }
            else if (level > 50 && level < 101)
            {
                await GoogleAuthentication.Instance.SaveTTCoin(60);
            }
            else if (level > 100 & level < 151)
            {
                await GoogleAuthentication.Instance.SaveTTCoin(80);
            }
            else if (level > 150)
            {
                await GoogleAuthentication.Instance.SaveTTCoin(100);
            }
        }
        ...
        /// </summary>
        private async void OpenNoMovesOrTimePopup()
        {
